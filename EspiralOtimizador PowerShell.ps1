...
$script:AppName = 'EspiralOtimizador'
...
function Get-LanguagePreference {
    $configPath = Join-Path -Path $env:APPDATA -ChildPath "EspiralOtimizador\language.json"
...
function Save-LanguagePreference {
    ...
    $configDir = Join-Path -Path $env:APPDATA -ChildPath "EspiralOtimizador"
...
function Get-AppIcon {
    param(
        [string]$IconUrl = 'EspiralOtimizador.ico'
    )
    
    try {
        # Criar diretorio AppData/Local/EspiralOtimizador se nao existir
        $appDataPath = [Environment]::GetFolderPath('LocalApplicationData')
        $EspiralOtimizadorPath = Join-Path $appDataPath 'EspiralOtimizador'
        $iconPath = Join-Path $EspiralOtimizadorPath 'EspiralOtimizador.ico'
        
        if (-not (Test-Path $EspiralOtimizadorPath)) {
            New-Item -Path $EspiralOtimizadorPath -ItemType Directory -Force | Out-Null
            Write-LogUI "Diretorio criado: $EspiralOtimizadorPath"
        }
...
function Get-AppIconPNG {
    param(
        [string]$IconUrl = 'EspiralOtimizador.png'
    )
    ...
    try {
        ...
        $appDataPath = [Environment]::GetFolderPath('LocalApplicationData')
        $EspiralOtimizadorPath = Join-Path $appDataPath 'EspiralOtimizador'
        $iconPath = Join-Path $EspiralOtimizadorPath 'EspiralOtimizador.png'
...
function New-RestorePoint {
    try {
        ...
        Checkpoint-Computer -Description 'EspiralOtimizador' -RestorePointType 'MODIFY_SETTINGS'
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"
        Title="EspiralOtimizador" Height="600" Width="900"
    ...
    <Grid>
        ...
        <Border Grid.Row="0" x:Name="TitleBar" Background="#0F0F0F" BorderBrush="#2B2B2B" BorderThickness="0,0,0,1">
            <Grid>
                ...
                <StackPanel Grid.Column="0" Orientation="Horizontal" VerticalAlignment="Center" Margin="12,0,0,0">
                    ...
                    <TextBlock Text="EspiralOtimizador" FontSize="14" FontWeight="Bold" Foreground="#FFFFFF" VerticalAlignment="Center"/>
                </StackPanel>
        ...
        <Border Grid.Row="1" Background="#111111" BorderBrush="#2B2B2B" BorderThickness="0,0,0,1" Padding="12">
            <DockPanel LastChildFill="False">
                <TextBlock Text="EspiralOtimizador" FontSize="24" FontWeight="Bold"/>
                            ...
                            <Grid>
                                ...
                                <StackPanel Grid.Column="0">
                                    <StackPanel Orientation="Horizontal" VerticalAlignment="Center">
                                        ...
                                        <TextBlock x:Name="txtAppName" Text="EspiralOtimizador" FontSize="24" FontWeight="Bold" Margin="8,0,0,0"/>
                                    </StackPanel>
                                    ...
                                    <Border Background="#141414" CornerRadius="8" Padding="12" Margin="0,10,0,0" BorderBrush="{StaticResource Border}" BorderThickness="1">
                                        <StackPanel>
                                            ...
                                            <TextBlock Text="CovexDev" Margin="0,6,0,0"/>
                                        </StackPanel>
                                    </Border>
                                </StackPanel>
...
$btnSaveLog.Add_Click({
    try {
        ...
        $dlg.FileName = "EspiralOtimizador-log-$(Get-Date -Format 'yyyyMMdd-HHmmss').txt"
...
function Invoke-SelectedOptimizations {
    ...
    if ($tasks.Count -eq 0) {
        [System.Windows.MessageBox]::Show('Nenhuma otimizacao selecionada.','EspiralOtimizador','OK','Information') | Out-Null
        return
    }
    ...
    try {
        ...
        [System.Windows.MessageBox]::Show($msg,'EspiralOtimizador','OK','Information') | Out-Null
    } finally {
        Set-UIBusy $false
...
function Invoke-DebloatOperations {
    ...
    if ($tasks.Count -eq 0) {
        [System.Windows.MessageBox]::Show('Nenhuma operacao de debloat selecionada.','EspiralOtimizador','OK','Information') | Out-Null
        return
    }

    $result = [System.Windows.MessageBox]::Show('Deseja criar um ponto de restauracao antes do debloat?','EspiralOtimizador','YesNo','Question')
    ...
    try {
        ...
        [System.Windows.MessageBox]::Show('Debloat concluido. Verifique o log para detalhes. Reinicie o sistema para aplicar todas as mudancas.','EspiralOtimizador','OK','Information') | Out-Null
    } finally {
        Set-UIBusy $false
        Start-Sleep -Milliseconds 500
        Set-Progress 0
...
function Invoke-SelectedOptimizationsNew {
    ...
    if ($tasks.Count -eq 0) {
        ...
        [System.Windows.MessageBox]::Show('Nenhuma otimizacao selecionada. Marque pelo menos uma opcao.','EspiralOtimizador - Erro','OK','Warning') | Out-Null
        return $false
    }
    ...
    if ($Simulate) {
        ...
        [System.Windows.MessageBox]::Show($msg, 'EspiralOtimizador - Simulacao', 'OK', 'Information') | Out-Null
        return $true
    }
    ...
    try {
        ...
        if ($results) {
            ...
            [System.Windows.MessageBox]::Show($msg, 'EspiralOtimizador - Resultado', 'OK', $msgType) | Out-Null
            
            return ($successCount -gt 0)
        } else {
            Write-LogUI "ERRO: Nenhum resultado retornado do Runspace" 'ERROR'
            [System.Windows.MessageBox]::Show("Erro inesperado: Nenhum resultado retornado.", 'EspiralOtimizador - Erro', 'OK', 'Error') | Out-Null
            return $false
        }
        
    } catch {
        Write-LogUI "ERRO CRITICO durante execucao das otimizacoes: $($_.Exception.Message)" 'ERROR'
        [System.Windows.MessageBox]::Show("Erro critico durante a execucao das otimizacoes:`n$($_.Exception.Message)", 'EspiralOtimizador - Erro Critico', 'OK', 'Error') | Out-Null
        return $false
    } finally {
        Set-UIBusy $false
        Start-Sleep -Milliseconds 500
...
function Invoke-DebloatOperationsNew {
    ...
    if ($tasks.Count -eq 0) {
        ...
        [System.Windows.MessageBox]::Show('Nenhuma operacao de debloat selecionada. Marque pelo menos uma opcao.','EspiralOtimizador - Erro','OK','Warning') | Out-Null
        return $false
    }
    ...
    if ($Simulate) {
        ...
        foreach ($n in $names) { Write-LogUI $n 'DEBUG' }
        [System.Windows.MessageBox]::Show($msg, 'EspiralOtimizador - Simulacao Debloat', 'OK', 'Information') | Out-Null
        return $true
    }
    ...
    try {
        ...
        if ($results) {
            ...
            $msgType = if ($failCount -eq 0) { 'Information' } elseif ($successCount -gt 0) { 'Warning' } else { 'Error' }
            [System.Windows.MessageBox]::Show($msg, 'EspiralOtimizador - Resultado do Debloat', 'OK', $msgType) | Out-Null
            
            return ($successCount -gt 0)
        } else {
            Write-LogUI "ERRO: Nenhum resultado retornado do Runspace de debloat" 'ERROR'
            [System.Windows.MessageBox]::Show("Erro inesperado: Nenhum resultado retornado do debloat.", 'EspiralOtimizador - Erro', 'OK', 'Error') | Out-Null
            return $false
        }
        
    } catch {
        Write-LogUI "ERRO CRITICO durante execucao do debloat: $($_.Exception.Message)" 'ERROR'
        [System.Windows.MessageBox]::Show("Erro critico durante a execucao do debloat:`n$($_.Exception.Message)", 'EspiralOtimizador - Erro Critico', 'OK', 'Error') | Out-Null
        return $false
    } finally {
        Set-UIBusy $false
        Start-Sleep -Milliseconds 500
...
function Invoke-ActiveTabOperationsNew {
    ...
    switch ($activeTabIndex) {
        ...
        1 { # Aba "Reverter"
            ...
            [System.Windows.MessageBox]::Show("A aba 'Reverter' nao possui operacoes automaticas.`nUse os botoes individuais para reverter configuracoes especificas.", "EspiralOtimizador - Informacao", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Information) | Out-Null
            return $false
        }
        2 { # Aba "Debloat"
        ...
        default {
            ...
            [System.Windows.MessageBox]::Show("Selecione a aba 'Geral' ou 'Debloat' para aplicar otimizacoes.", "EspiralOtimizador - Aviso", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Information) | Out-Null
            return $false
        }
    }
}
...
# Links da aba Sobre
if ($btnSite)      { $btnSite.Add_Click({ Start-Process 'EM BREVE' }) }
if ($btnPortfolio) { $btnPortfolio.Add_Click({ Start-Process 'EM BREVE' }) }
if ($btnYouTube)   { $btnYouTube.Add_Click({ Start-Process 'https://www.youtube.com/@covexstandoff2' }) }
if ($btnDiscord)   { $btnDiscord.Add_Click({ Start-Process 'EM BREVE' }) }
